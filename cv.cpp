...
int warp_affine(lua_State *L)
{
	...
	for (int i = 0; i < src_c; i++) {
		cv::Mat src_mat = cv::Mat(src_h, src_w, CV_32FC1, src + i * src_h * src_w);
		cv::Mat dst_mat = cv::Mat(dst_h, dst_w, CV_32FC1, dst + i * dst_h * dst_w);
		cv::warpAffine(src_mat, dst_mat, warp_mat, dst_mat.size(), CV_INTER_CUBIC);
	}
